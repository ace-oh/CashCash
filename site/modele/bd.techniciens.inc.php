...
function getAllDistancesToClients($technicianId) {
    try {
        $cnx = connexionPDO();
        $query = "SELECT client.idClient, client.distanceKm
                  FROM client
                  INNER JOIN intervention ON client.idClient = intervention.clientInter
                  WHERE intervention.idTech = :technicianId";
        $stmt = $cnx->prepare($query);
        $stmt->bindParam(':technicianId', $technicianId, PDO::PARAM_STR);
        $stmt->execute();
